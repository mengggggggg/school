    ...
    public class BackOrderFOController : Controller
    {
        ...
        public ActionResult SelectforOrder()
        {
            ...
            try
            {
                ...
                sql = "select A.Order_ID,Order_Total_Amount,Order_Status,Order_Date,Order_Address,Order_Pay_Way,Order_Delivery_Way,Order_Note,A.Member_Account from Order_Data A left join Order_Details_Data B on A.Order_ID = B.Order_ID where B.Store_ID = @Store_ID and B.Store_Name=@Store_Name";
        ...
        public ActionResult uptForOrder(string Order_ID)
        {
            if (Session["Member_identity"] != null)
            {
                if (Session["Member_identity"].ToString() == "商家")
        ...
        public ActionResult uptForOrder(Cart inModel, string SelectStatus, string SelectPay_Way, string SelectDelivery_Way)
        {
            if (Session["Member_identity"] != null)
            {
                if (Session["Member_identity"].ToString() == "商家")
            ...
            try
            {
                ...
                if (Ret > 0)
                {
                    TempData["uomsg"] = "";
                    TempData["uomsg"] = "更新成功";
                    return RedirectToAction("SelectforOrder", "BackOrderFO");
                }
                else
                {
                    TempData["uomsg"] = "";
        ...
        public ActionResult DetailForOrder(string Order_ID)
        {
            if (Session["Member_identity"] != null)
            {
                if (Session["Member_identity"].ToString() == "商家")
            ...
            if (Order_ID == null)
            {
                return RedirectToAction("SelectforOrder");
            }
            SqlConnection conn = null;
            try
            {
                ...
                Session["Back_ForOrder_ID"] = Order_ID;
                var model = new List<Cart>();
                conn.Open();
                SqlDataReader s = cmd.ExecuteReader();
                while (s.Read())
        ...
        public ActionResult delForOrder(string Order_ID)
        {
            if (Session["Member_identity"] != null)
            {
                if (Session["Member_identity"].ToString() == "商家")
            ...
            BackOrderFOController OrderController = new BackOrderFOController();
            OrderController.delForOrderById(Order_ID);
            TempData["delo"] = "刪除成功";
            return RedirectToAction("SelectforOrder");
        }

        public void delForOrderById(string Order_ID)
        {
